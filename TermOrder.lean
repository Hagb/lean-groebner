...
namespace TermOrder
variable [α_add: Add α] [α_zero: Zero α] [α_add_cancel_comm_monoid: AddCancelCommMonoid α]
...
instance toAddCancelCommMonoid: AddCancelCommMonoid (TermOrder α) := α_add_cancel_comm_monoid
instance toAddCommMonoid: AddCommMonoid (TermOrder α) := α_add_cancel_comm_monoid.toAddCommMonoid
...
instance:
  CovariantClass α α (·+·) (·≤·) :=
                                      TermOrderClass.toCovariantClass
...
instance:
  CovariantClass α α (·+·) (·<·) :=
    @AddLeftCancelSemigroup.covariant_add_lt_of_covariant_add_le α _ _ TermOrderClass.toCovariantClass
...
variable [TermOrderClass (TermOrder (σ→₀ℕ))]

lemma TermOrder.le_of_finsupp_le {k₁ k₂: σ→₀ℕ} (h: k₁≤k₂): LE.le (α:=TermOrder (σ→₀ℕ)) k₁ k₂ := by
  -- have key: k₁+(k₂-k₁)=k₂ := by
  --   -- library_search
  rw [←Iff.mpr add_tsub_cancel_iff_le h]
  -- simp?
  simp only [ge_iff_le, le_add_iff_nonneg_right]
  exact ZeroLEClass.zero_le _

lemma TermOrder.lt_of_finsupp_lt {k₁ k₂: σ→₀ℕ} (h: k₁<k₂): LT.lt (α:=TermOrder (σ→₀ℕ)) k₁ k₂
  := lt_of_le_of_ne (TermOrder.le_of_finsupp_le (le_of_lt h)) (ne_of_lt (α:=σ→₀ℕ) h)
